...
// Normalizes whatever shape the API returns into a plain array of cart items.
// Handles: array response, { items: [...] } response, { items: {...} } (object map),
// or anything else unexpected — always falls back to [].
...
export default function MyCartScreen({ navigation }) {
  ...
  const loadCart = () => {
    setLoading(true);
    apiRequest('/cart')
      .then((data) => setItems(normalizeCartItems(data)))
  ...
  const updateQuantity = async (id, delta) => {
    ...
    try {
      ...
      if (updatedCart) setItems(normalizeCartItems(updatedCart));
    } catch (err) {
            ...
            <View style={styles.itemInfo}>
              ...
              <View style={styles.itemMetaRow}>
                ...
                <View style={styles.stepper}>
                  <TouchableOpacity style={styles.stepperButton} onPress={() => updateQuantity(item.id, -1)}>
