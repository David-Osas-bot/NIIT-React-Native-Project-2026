...
export default function RestaurantViewScreen() {
  ...
  const handleCardPress = (item) => {
    navigation.navigate('FoodDetailScreen', {
      food: {
        ...item,
        // FoodDetailScreen expects a remote `imageUrl` string for <Image
        // source={{ uri }}>. These menu items only have a local `image`
        // require() asset, so there's no real imageUrl yet — pass the
        // local asset through separately so FoodDetailScreen can fall back
        // to <Image source={item.image}> instead of showing its "no image"
        // placeholder. See the note left in FoodDetailScreen if you want to
        // wire that fallback in.
        imageUrl: item.imageUrl ?? null,
        image: item.image,
        restaurantName: activeRestaurant.name,
        rating: activeRestaurant.rating,
        deliveryFee: activeRestaurant.delivery === 'Free' ? 0 : activeRestaurant.delivery,
        prepTime: activeRestaurant.time,
      },
    });
  };
  ...
  const handleAddToCart = async (item) => {
    setAddingId(item.id);
    try {
      // Confirmed schema (matches MyCartScreen): POST /cart/items body ->
      // { id/name/price/quantity, ... }. Adjust the payload shape here if
      // your API expects something different.
